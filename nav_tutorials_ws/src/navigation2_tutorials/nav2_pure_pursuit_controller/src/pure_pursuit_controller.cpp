  ...
  const std::shared_ptr<nav2_costmap_2d::Costmap2DROS> & costmap_ros)
{
  ...
  RCLCPP_INFO(
    logger_,
    "Configured pure_pursuit_controller::PurePursuitController");
  ...
  const geometry_msgs::msg::Twist &)
{
  ...
  RCLCPP_INFO(
    logger_,
    "computeVelocityCommand Step1");

    
  local_plan_ = transformGlobalPlan(global_plan_, "base_link");


auto goal_pose = local_plan_.poses.back().pose;
/*
auto goal_pose = std::find_if(
    local_plan_.poses.begin(), local_plan_.poses.end(),
    [&](const auto & local_plan_pose) {
      return hypot(
        local_plan_pose.pose.position.x,
        local_plan_pose.pose.position.y) >= lookahead_dist_;
    })->pose;
*/
/*
...
*/
  ...
  RCLCPP_INFO(
    logger_,
    "Goal position.x %f ", goal_pose.position.x);


  RCLCPP_INFO(
    logger_,
    "Goal position.y %f ", goal_pose.position.y);
...
*/
  ...
  RCLCPP_INFO(
    logger_,
    "linear_vel %f ", cmd_vel.twist.linear.x);

  RCLCPP_INFO(
    logger_,
    "angular_vel %f ", cmd_vel.twist.angular.z);

  RCLCPP_INFO(
    logger_,
    "time_nsec:  %lu ", (unsigned long int) now.nanoseconds());
...
void PurePursuitController::setPlan(const nav_msgs::msg::Path & path)
{
  ...
  global_plan_ = transformGlobalPlan(path, "odom");
}

nav_msgs::msg::Path PurePursuitController::transformGlobalPlan(const nav_msgs::msg::Path & path, const char* frame_id_to)
{
  ...
  transform_dist_threshold = 1.0;

  RCLCPP_INFO(
    logger_,
    "Transform thereshold calculated");

  // Transform the near part of the global plan into the robot's frame of reference.
  nav_msgs::msg::Path transformed_plan;
  transformed_plan.header.frame_id = frame_id_to;//"base_link";//costmap_ros_->getBaseFrameID();
...
RCLCPP_INFO(
    logger_,
    "Threshold %lf", transform_dist_threshold);
